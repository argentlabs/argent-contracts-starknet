...
struct PluginContext:
    member plugin_id: felt
    member plugin_data_len: felt
    member plugin_data: felt*
end
...
@storage_var
func _plugin_context() -> (res: PluginContext):
end
...
func use_plugin{
    ...
    let (authorised) = _plugins.read(plugin)
    with_attr error_message("unauthorised plugin"):
        assert_not_zero(authorised)
    end
    # store plugin context
    let plugin_context: PluginContext = PluginContext(plugin_id, plugin_data_len, plugin_data)
    _plugin_context.write(plugin_context)
end

func get_plugin_context
