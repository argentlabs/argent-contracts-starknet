...
fn test_trigger_escape_first_signer() {
    ...
    assert(*escape.new_signers.at(0) == MULTISIG_OWNER(3), 'should be signer 1');
...
fn test_trigger_escape_all_signers() {
    ...
    let (escape, status) = component.get_escape();
    assert(*escape.target_signers.at(0) == MULTISIG_OWNER(1), 'should be signer 1');
    assert(*escape.target_signers.at(1) == MULTISIG_OWNER(2), 'should be signer 2');
    assert(*escape.new_signers.at(0) == MULTISIG_OWNER(4), 'should be signer 4');
    assert(*escape.new_signers.at(1) == MULTISIG_OWNER(3), 'should be signer 3');
    assert(escape.ready_at == 10, 'should be 10');
    assert(status == EscapeStatus::NotReady, 'should be NotReady');
}
