...
//
//   before(async () => {
//     moAccountClassHash = await manager.declareLocalContract("MultiOwnerAccount");
//   });
//
//   beforeEach(async () => {
...
//       const { account, accountContract, guardian } = await deployMoAccount();
...
//           });
//
//           it(`Expect to be able to trigger /w multiple owners`, async function () {
//             const { account, accountContract, other } = await buildAccount(type);
//
...
//       const upgradeReceipt = await upgradeAccount(account, moAccountClassHash, ["0"]);
//
...
//
//       account.signer = new LegacyMultisigSigner([owner, guardian]);
//       await upgradeAccount(account, moAccountClassHash, ["0"]);
//       await getEscapeStatus(await manager.loadContract(account.address)).should.eventually.equal(EscapeStatus.None);
//     });
//
//     describe("Testing with all guardian signer combination", function () {
...
//       const { account, accountContract, owner } = await deployMoAccount();
//       account.signer = new ArgentSigner(owner);
//
//       await manager.setTime(randomTime);
//       const readyAt = BigInt(randomTime) + ESCAPE_SECURITY_PERIOD;
