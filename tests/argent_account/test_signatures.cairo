...
fn valid_no_guardian() {
    ...
    assert_eq!(account.is_valid_signature(tx_hash, signatures));
}

#[test]
fn valid_with_guardian() {
    let signatures = to_starknet_signatures(array![OWNER(), GUARDIAN()]);
    assert_eq!(initialize_account().is_valid_signature(tx_hash, signatures));
}
...
fn invalid_hash_1() {
    let account = initialize_account_without_guardian();
    let signatures = to_starknet_signatures(array![OWNER()]);
    assert_eq!(account.is_valid_signature(0, signatures));
}
...
fn invalid_hash_2() {
    let account = initialize_account_without_guardian();
    let signatures = to_starknet_signatures(array![OWNER()]);
    assert_eq!(account.is_valid_signature(123, signatures));
}
