...
describe("ArgentAccount: multicall", function () {
  ...
  it("Should be possible to send eth", async function () {
    ...
    const senderInitialBalance = await provider.ethBalance(account.address);
    const recipientInitialBalance = await provider.ethBalance(recipient);
    ethContract.connect(account);
    const { transaction_hash } = await ethContract.transfer(recipient, amount);
    await account.waitForTransaction(transaction_hash);

    const senderFinalBalance = await provider.ethBalance(account.address);
    const recipientFinalBalance = await provider.ethBalance(recipient);
  ...
  it("Should be possible to send eth with a Cairo1 account using a multicall", async function () {
    ...
    const senderInitialBalance = await provider.ethBalance(account.address);
    const recipient1InitialBalance = await provider.ethBalance(recipient1);
    const recipient2InitialBalance = await provider.ethBalance(recipient2);
    ...
    const senderFinalBalance = await provider.ethBalance(account.address);
    const recipient1FinalBalance = await provider.ethBalance(recipient1);
    const recipient2FinalBalance = await provider.ethBalance(recipient2);
