...
mod MultiOwnerAccountT {
    ...
    use super::super::account_interface::{IArgentMultiOwnerAccountT,};
    ...
    #[constructor]
    fn constructor(ref self: ContractState, owners: Array<Signer>, threshold: usize, guardian: Option<Signer>) {
        self.owner_manager.initialize(owners, threshold);
    ...
    #[abi(embed_v0)]
    impl ArgentMultiOwnerAccountTImpl of IArgentMultiOwnerAccountT<ContractState> {
        ...
        fn change_threshold(ref self: ContractState, new_threshold: usize) {
            // TODO we might need to make sure that the account is not bricked, maybe we need to require a valid
            // signature to prove it
            assert_only_self();
            self.owner_manager.change_threshold(new_threshold);
            self.reset_escape();
            self.reset_escape_timestamps();
        }

        fn add_owners(ref self: ContractState, new_owners: Array<Signer>, new_threshold: usize) {
            assert_only_self();
            self.owner_manager.add_owners(new_owners, new_threshold);
            self.reset_escape();
            self.reset_escape_timestamps();
        }

        fn remove_owners(ref self: ContractState, owner_guids_to_remove: Array<felt252>, new_threshold: usize) {
            assert_only_self();
            // TODO we might need to make sure that the account is not bricked, maybe we need to require a valid
            // signature to prove it
            self.owner_manager.remove_owners(owner_guids_to_remove, new_threshold);
