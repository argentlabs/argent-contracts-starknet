use argent::signer::signer_signature::{Secp256k1Signature, SignerTrait, StarknetSigner, StarknetSignature, starknet_signer_from_pubkey};
use ecdsa::check_ecdsa_signature;
use hash::{HashStateTrait};
use pedersen::{PedersenTrait};
use snforge_std::signature::{
    KeyPair, KeyPairTrait,
...
fn NEW_OWNER() -> (StarknetSigner, StarknetSignature) {
    let new_owner_message_hash = new_owner_message_hash('SN_GOERLI', ARGENT_ACCOUNT_ADDRESS, starknet_signer_from_pubkey(OWNER().pubkey).into_guid());
