...
describe("ArgentAccount", function () {
  ...
  it("Deploy externally", async function () {
    ...
    await accountContract.get_owners_guid().should.eventually.deep.equal([owner.guid]);
    ...
    await accountContract.get_guardians_guid().should.eventually.deep.equal([guardian.guid]);
  });
  ...
  for (const useTxV3 of [false, true]) {
    it(`Self deployment (TxV3: ${useTxV3})`, async function () {
      ...
      await accountContract.get_owners_guid().should.eventually.deep.equal([owner.guid]);
      await accountContract.get_guardians_guid().should.eventually.deep.equal([]);
    });
  }
  ...
  describe("reset_owners(...)", function () {
    it("Should be possible to reset_owners", async function () {
      ...
      await accountContract.get_owners_guid().should.eventually.deep.equal([newOwner.guid]);
    });
