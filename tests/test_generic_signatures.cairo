use argent::common::{
    ...
    signer_signature::{
        Signer, SignerSignatureTrait, SignerSignature, StarknetSignature, StarknetSigner, Secp256k1Signer,
        Secp256r1Signer, WebauthnSigner, IntoGuid
    },
...
fn initialize_account(signer: Signer) -> IAccountDispatcher {
    let mut calldata = array![1];
    array![signer].serialize(ref calldata);
...
fn test_valid_signature_starknet() {
    let owner = StarknetSigner { pubkey: owner_pubkey };
    let signer_signature = SignerSignature::Starknet((owner, StarknetSignature { r: owner_r, s: owner_s }));
    let signatures = array![signer_signature];
    assert(
        initialize_account(Signer::Starknet(owner))
            .is_valid_signature(message_hash, serialize(@signatures)) == VALIDATED,
        'invalid signature'
    );
}
...
fn test_valid_signature_secp256k1() {
    let owner = Secp256k1Signer { pubkey_hash: EthAddress { address: owner_pubkey_eth } };
    let signer_signature = SignerSignature::Secp256k1(
        (owner, Signature { r: owner_eth_r, s: owner_eth_s, y_parity: owner_eth_v % 2 == 0 })
    );
    let signatures = array![signer_signature];
    assert(
        initialize_account(Signer::Secp256k1(owner))
            .is_valid_signature(message_hash, serialize(@signatures)) == VALIDATED,
        'invalid signature'
    );
}
...
fn test_valid_signature_webauthn_1() {
    ...
    assert(
        initialize_account(Signer::Webauthn(signer)).is_valid_signature(challenge, serialize(@signatures)) == VALIDATED,
        'invalid signature'
    );
}
