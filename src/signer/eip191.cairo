...
fn calculate_eip191_hash(message: felt252) -> u256 {
    ...
    let shift_4_bytes: NonZero<u256> = integer::u256_try_as_non_zero(0x100000000).unwrap();
    let shift_8_bytes: NonZero<u256> = integer::u256_try_as_non_zero(0x10000000000000000).unwrap();

    let rest: u256 = message.into();
    let (rest, tx_hash_part_5) = integer::u256_safe_div_rem(rest, shift_4_bytes);
    let (rest, tx_hash_part_4) = integer::u256_safe_div_rem(rest, shift_8_bytes);
    let (rest, tx_hash_part_3) = integer::u256_safe_div_rem(rest, shift_8_bytes);
    let (tx_hash_part_1, tx_hash_part_2) = integer::u256_safe_div_rem(rest, shift_8_bytes);
...
fn to_le(input: u64) -> u64 {
    let result_u128 = integer::u128_byte_reverse(input.into());
