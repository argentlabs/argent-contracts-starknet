...
import { declareContract, deployAccount, deployer, dump, load, provider, removeFromCache, restart } from "./lib";
...
describe("Declare", function () {
  ...
  it("Expect the account to be able to declare a Cairo contract version1 ", async function () {
    const { account } = await deployAccount(argentAccountClassHash);
    const contract: CompiledSierra = json.parse(readFileSync("./tests/fixtures/Proxy.json").toString("ascii"));
    const { class_hash, transaction_hash } = await account.declare({ contract });
    await deployer.waitForTransaction(transaction_hash);
    expect(provider.getCompiledClassByClassHash(class_hash)).to.exist;
  });

  it("Expect the account to be able to declare a Cairo contract version2 ", async function () {
    const { account } = await deployAccount(argentAccountClassHash);
