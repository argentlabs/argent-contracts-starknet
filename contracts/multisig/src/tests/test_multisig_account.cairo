...
use multisig::{ArgentMultisig, MultisigStorage};
...
fn valid_before_init() {
    assert(ArgentMultisig::get_signers().is_empty(), 'invalid signers length');
}
...
fn valid_initiliaze() {
    ...
    assert(ArgentMultisig::is_signer(signer_pubkey_1), 'is signer cant find signer');
...
fn valid_initiliaze_two_signers() {
    ...
    ArgentMultisig::constructor(threshold, signers_array);
    // test if is signer correctly returns true
    assert(ArgentMultisig::is_signer(signer_pubkey_1), 'is signer cant find signer 1');
    assert(ArgentMultisig::is_signer(signer_pubkey_2), 'is signer cant find signer 2');
...
fn change_threshold() {
    ...
    ArgentMultisig::change_threshold(2);
    assert(ArgentMultisig::get_threshold() == 2, 'new threshold not set');
}
...
fn add_signers() {
    ...
    ArgentMultisig::add_signers(2, new_signers);
...
#[test]
fn get_name() {
    assert(ArgentMultisig::get_name() == 'ArgentMultisig', 'Name should be ArgentMultisig');
}

#[test]
fn get_version() {
    let version = ArgentMultisig::get_version();
